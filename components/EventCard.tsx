...
const EventCard = ({ event }: EventCardProps) => {
  ...
  const handleJoinClick = (e: React.MouseEvent) => {
    e.preventDefault(); // Предотвращаем переход по ссылке Link
  ...
  return (
    <div className="w-[210px] group cursor-pointer">
      <Link href={`/event/${event.id}`} passHref>
        <div className="relative w-full h-[200px] mb-[10px] transition-transform duration-200 group-hover:scale-[1.03]">
        ...
        <div className="flex flex-col gap-[20px]">
          <div className="flex flex-col">
            <h3 className="text-[13px] font-medium leading-[20px] mb-1 group-hover:text-green-600 transition-colors">{event.title}</h3>
          ...
          <div className="flex flex-col gap-[4px]">
            ...
            <div className="mt-[20px]">
              <button 
                className="w-full h-[30px] bg-green-100 text-green-800 text-[13px] font-medium rounded-full hover:bg-green-200 transition-colors"
